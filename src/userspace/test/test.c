...
/* test.elf -- Phase 11.x regression suite.
 *
 * Spawned by init.elf via SYS_SPAWN with a bootstrap channel.
 * Returns 0 on success, or the first non-zero exit code from a
 * failed sub-test.  The spawn/wait test lives in init.elf (the
 * parent), not here -- otherwise this binary would recurse. */
...
int main(int argc, char **argv, char **envp) {
    ...
    printf("test: hello from userspace (C)\n");

    /* --- abs / labs --- */
    printf("test: abs    [%d] [%d] [%ld] [%ld]\n",
    ...
    /* --- string.h tail regression (Phase 11.3.2) --- */
    {
        ...
        printf("test: strspn/cspn [%zu] [%zu] [%zu] [%zu]\n",
               s1, s2, s3, s4);
        ...
        printf("test: strncpy/strncat ok\n");
    }

    /* --- numeric conversion regression --- */
    printf("test: strtol [%ld] [%ld] [%ld]\n",
           strtol("-17", NULL, 0),
           strtol("0xff", NULL, 0),
           strtol("101010", NULL, 2));

    /* --- strerror / perror --- */
    printf("test: strerror [%s] [%s] [%s] [%s]\n",
           strerror(ENOENT),
           strerror(EINVAL),
           strerror(ERANGE),
           strerror(ENOSYS));
    ...
    if (!a || !b || !c) { printf("test: malloc failed\n"); return 1; }
    strcpy(a, "hello, malloc!");
    strcpy(b, "block-b");
    strcpy(c, "block-c");
    printf("test: a=%s b=%s c=%s\n", a, b, c);

    free(b);
    free(a);

    char *d = (char *)malloc(96);
    if (!d) { printf("test: re-malloc failed\n"); return 2; }
    strcpy(d, "reused");
    printf("test: d=%s c=%s\n", d, c);
    if (strcmp(c, "block-c") != 0) {
        printf("test: coalesce test failed, c=%s\n", c);
        return 3;
    }

    unsigned char *z = (unsigned char *)calloc(32, 1);
    if (!z) { printf("test: calloc failed\n"); return 4; }
    for (int i = 0; i < 32; ++i) {
        if (z[i] != 0) {
            printf("test: calloc not zero at index %d\n", i);
            return 5;
        }
    }
    free(z);
    ...
    if (!r) { printf("test: realloc setup failed\n"); return 6; }
    strcpy(r, "small");
    r = (char *)realloc(r, 128);
    if (!r || strcmp(r, "small") != 0) {
        printf("test: realloc grow failed\n");
        return 7;
    }
    free(r);
    free(c);
     ...
     */
    {
        ...
        printf("test: env setenv overwrite [%d] [%s]\n",
               r4, path2 ? path2 : "(null)");
        ...
        printf("test: env setenv new [%d] [%s]\n",
               r5, e1 ? e1 : "(null)");
        ...
        for (int i = 0; i < 20; ++i) {
            ...
            if (setenv(nm, vl, 1) != 0) {
                printf("test: env growth setenv failed at %d\n", i);
                return 20;
            }
        }
        for (int i = 0; i < 20; ++i) {
            ...
            if (!got || strcmp(got, want) != 0) {
                printf("test: env growth verify failed at %d\n", i);
                return 21;
            }
        }
        const char *path3 = getenv("PATH");
        if (!path3 || strcmp(path3, "/new") != 0) {
            printf("test: env PATH lost after growth\n");
            return 22;
        }
        printf("test: env growth ok (20 vars, PATH preserved)\n");

        for (int i = 0; i < 20; ++i) {
            char nm[32];
            snprintf(nm, sizeof nm, "EXYDE_TEST_%d", i);
            if (unsetenv(nm) != 0) {
                printf("test: env shrink unsetenv failed at %d\n", i);
                return 23;
            }
        }
        ...
        int e11 = errno;
        if (r11 != -1 || e11 != ENOMEM) {
            printf("test: env enomem unexpected [%d] [%d] holes=%d\n",
                   r11, e11, n_holes);
            return 24;
        }
        if (getenv("EXYDE_ENOMEM")) {
            printf("test: env enomem left a stale entry\n");
            return 25;
        }
        printf("test: env enomem ok (holes=%d)\n", n_holes);

        /* (12) getenv still works after everything. */
        const char *path4 = getenv("PATH");
        printf("test: env final PATH [%s]\n", path4 ? path4 : "(null)");
    ...
    /* --- microkernel ABI regression (Phase 11.5.0) --- */
    {
        ...
        if (ch == EXYDE_HANDLE_INVALID) {
            printf("test: micro FAIL ipc_create errno=%d\n", errno);
            return 30;
        }
        ...
        if (!p) {
            printf("test: micro FAIL map errno=%d\n", errno);
            return 43;
        }
        p[0] = 0xAB;
        ...
        printf("test: micro map/yield ok\n");
        ...
        if (xch == EXYDE_HANDLE_INVALID || data_ch == EXYDE_HANDLE_INVALID)
            { printf("test: micro cap FAIL code=60\n"); return 60; }
        ...
        /* --- TRANSFER --- */
        {
            ...
            if (exyde_ipc_send_cap(xch, msg, 16, data_ch,
                ...
                { printf("test: micro cap FAIL code=61\n"); return 61; }

            /* Sender's old handle must now be dead. */
            char dummy[16] = "z";
            errno = 0;
            if (exyde_ipc_try_send(data_ch, dummy, 16) != -1 || errno != EBADF)
                { printf("test: micro cap FAIL code=62\n"); return 62; }

            /* Receiver picks up the capability. */
            char rx[16];
            exyde_handle_t got = EXYDE_HANDLE_INVALID;
            if (exyde_ipc_recv_cap(xch, rx, 16, &got) != 16) { printf("test: micro cap FAIL code=63\n"); return 63; }
            if (strcmp(rx, "xfer") != 0) { printf("test: micro cap FAIL code=64\n"); return 64; }
            if (got == EXYDE_HANDLE_INVALID) { printf("test: micro cap FAIL code=65\n"); return 65; }

            /* New handle works: send + recv on it. */
            char p1[16] = "hi";
            if (exyde_ipc_try_send(got, p1, 16) != 0) { printf("test: micro cap FAIL code=66\n"); return 66; }
            char p2[16];
            if (exyde_ipc_recv(got, p2, 16) != 16) { printf("test: micro cap FAIL code=67\n"); return 67; }
            if (strcmp(p2, "hi") != 0) { printf("test: micro cap FAIL code=68\n"); return 68; }

            if (exyde_handle_close(got) != 0) { printf("test: micro cap FAIL code=69\n"); return 69; }
        }

        /* --- DUPLICATE --- */
        {
            exyde_handle_t dup_src = exyde_ipc_create(16, 1);
            if (dup_src == EXYDE_HANDLE_INVALID) { printf("test: micro cap FAIL code=70\n"); return 70; }

            char msg[16] = "dup";
            if (exyde_ipc_send_cap(xch, msg, 16, dup_src,
                                   EXYDE_IPC_CAP_DUPLICATE) != 0)
                { printf("test: micro cap FAIL code=71\n"); return 71; }

            /* Sender's original handle must still work. */
            char p1[16] = "keep";
            if (exyde_ipc_try_send(dup_src, p1, 16) != 0) { printf("test: micro cap FAIL code=72\n"); return 72; }

            /* Drain that to keep the channel clean. */
            char p2[16];
            if (exyde_ipc_recv(dup_src, p2, 16) != 16) { printf("test: micro cap FAIL code=73\n"); return 73; }
            if (strcmp(p2, "keep") != 0) { printf("test: micro cap FAIL code=74\n"); return 74; }

            /* Receiver gets a copy. */
            char rx[16];
            exyde_handle_t got = EXYDE_HANDLE_INVALID;
            if (exyde_ipc_recv_cap(xch, rx, 16, &got) != 16) { printf("test: micro cap FAIL code=75\n"); return 75; }
            if (strcmp(rx, "dup") != 0) { printf("test: micro cap FAIL code=76\n"); return 76; }
            if (got == EXYDE_HANDLE_INVALID) { printf("test: micro cap FAIL code=77\n"); return 77; }

            /* Both handles now usable independently. */
            if (exyde_ipc_try_send(got, p1, 16) != 0) { printf("test: micro cap FAIL code=78\n"); return 78; }
            /* got and dup_src refer to the SAME channel (DUPLICATE);
             * it now holds one message, so a second try_send must
             * fail with EAGAIN. */
            errno = 0;
            if (exyde_ipc_try_send(dup_src, p1, 16) != -1 || errno != EAGAIN) { printf("test: micro cap FAIL code=79\n"); return 79; }
            char p3[16];
            if (exyde_ipc_recv(dup_src, p3, 16) != 16) { printf("test: micro cap FAIL code=79b\n"); return 79; }
            if (strcmp(p3, "keep") != 0) { printf("test: micro cap FAIL code=79c\n"); return 79; }

            if (exyde_handle_close(got) != 0) { printf("test: micro cap FAIL code=80\n"); return 80; }
            if (exyde_handle_close(dup_src) != 0) { printf("test: micro cap FAIL code=81\n"); return 81; }
        }

        /* --- plain recv on a cap-carrying message refuses (EINVAL) --- */
        {
            exyde_handle_t tag = exyde_ipc_create(16, 1);
            if (tag == EXYDE_HANDLE_INVALID) { printf("test: micro cap FAIL code=82\n"); return 82; }

            char msg[16] = "z";
            if (exyde_ipc_send_cap(xch, msg, 16, tag,
                                   EXYDE_IPC_CAP_DUPLICATE) != 0)
                { printf("test: micro cap FAIL code=83\n"); return 83; }

            char rx[16];
            errno = 0;
            if (exyde_ipc_try_recv(xch, rx, 16) != -1 || errno != EINVAL)
                { printf("test: micro cap FAIL code=84\n"); return 84; }

            /* The message must still be in the channel; consume it
             * properly this time. */
            exyde_handle_t got = EXYDE_HANDLE_INVALID;
            if (exyde_ipc_recv_cap(xch, rx, 16, &got) != 16) { printf("test: micro cap FAIL code=85\n"); return 85; }
            if (got == EXYDE_HANDLE_INVALID) { printf("test: micro cap FAIL code=86\n"); return 86; }

            if (exyde_handle_close(got) != 0) { printf("test: micro cap FAIL code=87\n"); return 87; }
            if (exyde_handle_close(tag) != 0) { printf("test: micro cap FAIL code=88\n"); return 88; }
        }
        ...
        /* --- send without TRANSFER right -> EPERM --- */
        {
            ...
            if (th < 0) { printf("test: micro cap FAIL code=89\n"); return 89; }

            char msg[16] = "no";
            errno = 0;
            if (exyde_ipc_send_cap(xch, msg, 16, (exyde_handle_t)th,
                                   EXYDE_IPC_CAP_TRANSFER) != -1 || errno != EPERM)
                { printf("test: micro cap FAIL code=90\n"); return 90; }

            if (exyde_handle_close((exyde_handle_t)th) != 0) { printf("test: micro cap FAIL code=91\n"); return 91; }
        }

        /* --- bad action -> EINVAL --- */
        {
            exyde_handle_t tmp = exyde_ipc_create(16, 1);
            if (tmp == EXYDE_HANDLE_INVALID) { printf("test: micro cap FAIL code=92\n"); return 92; }

            char msg[16] = "a";
            errno = 0;
            if (exyde_ipc_send_cap(xch, msg, 16, tmp, 99) != -1 || errno != EINVAL)
                { printf("test: micro cap FAIL code=93\n"); return 93; }

            if (exyde_handle_close(tmp) != 0) { printf("test: micro cap FAIL code=94\n"); return 94; }
        }

        if (exyde_handle_close(xch) != 0) { printf("test: micro cap FAIL code=95\n"); return 95; }

        printf("test: micro ipc cap ok\n");
